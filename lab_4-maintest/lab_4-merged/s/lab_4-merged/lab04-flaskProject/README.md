...
### Локальний запуск
1. Встановіть Python 3.10+ та pip.
2. Клонуйте репозиторій.
3. Встановіть залежності: `pip install -r requirements.txt`
4. Запустіть додаток: `python app.py` або `flask run`
5. Відкрийте http://localhost:5000

### Запуск у Docker
1. Встановіть Docker та Docker Compose.
2. Запустіть: `docker-compose up --build`
3. Додаток буде доступний на http://localhost:5000

### Конфігурація
- **Змінні середовища** (у .env файлі):
  - `FLASK_ENV`: середовище (development/production)
  - `FLASK_APP`: основний файл додатку (app.py)
  - `FLASK_RUN_HOST`: хост для запуску (0.0.0.0 для контейнера)
- **База даних**: SQLite файл `db.sqlite` зберігається у volume для збереження даних між запусками.
- **Health check**: Додаток має ендпоінт `/health` для перевірки стану.
